...
| `no_answer` | трубку не берут | недозвон |
| `contacted` | разговор состоялся | в работе |
| `in_progress` | переговоры продолжаются | в работе |
| `qualified` | целевой, готов двигаться дальше | в работе |
...
| `invalid` | чужой город, ошибочный номер, спам | потеряны |

Шаг воронки «обработан» = **был живой контакт** (`isReached`). Отказ сюда входит —
с человеком поговорили; недозвон и нецелевой не входят. Поэтому продажи всегда
вложены в обработанных, и воронка не «расширяется» на среднем шаге.
