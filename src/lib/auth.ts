...
/**
 * Отдел продаж: директор и РОП.
 *
 * РОП руководит менеджерами и продажниками — он их набирает, ведёт их график
 * и смотрит показатели каждого. Без этого руководить нечем. Остальные разделы
 * руководства (настройки, интеграции, деньги компании) ему по-прежнему
 * закрыты: он ведёт отдел, а не компанию.
 *
 * Это удобство, а не защита. Что именно РОПу позволено менять, решают
 * политики базы: заводить и увольнять он может только подчинённых.
 */
