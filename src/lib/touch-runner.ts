 ...
 *
 * Первая сейчас вхолостую: кнопку, которой менеджер назначал себе обещание,
 * из бота убрали — она стояла на пути и мешала отмечать статус одним
 * касанием. Механика оставлена целиком: клиенты не теряются и без будильника,
 * они лежат в своих пачках в «Моих клиентах», а если напоминания понадобятся
 * снова, вернуть надо будет одну кнопку, а не эту работу.
 *
 ...
 *
 * «Пробный» здесь обязателен: после урока продажник обещает вернуться к
 * клиенту, а заявка при этом остаётся на шаге урока — исход занятия статус
 * клиента не переписывает. Без этой строки его обещание молча пропадало бы.
 */
const OPEN_STATUSES = ['new', 'no_answer', 'thinking', 'trial'];
